...
import { FileText, Award, Eye, Users, GraduationCap } from "lucide-react";
...
export function TutorMockExamResults() {
  const [selectedResult, setSelectedResult] = useState<TutorExamResultData | null>(null);

  const { data: results = [], isLoading } = useQuery<TutorExamResultData[]>({
    queryKey: ["/api/tutor/mock-exam-results"],
  });
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Student Mock Exam Results
          </CardTitle>
          <CardDescription>View your students' mock exam performance</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground text-center py-6">
            No mock exam results available for your students yet
          </p>
        </CardContent>
      </Card>
    );
  ...
  const hasResults = (result: TutorExamResultData) => {
    return result.statistics.length > 0 && result.statistics.some(s => s.studentScore !== null);
  };
  ...
  const resultsByStudent = results.reduce((acc, result) => {
    ...
    acc[result.studentId].exams.push(result);
    return acc;
  }, {} as Record<string, { studentName: string; exams: TutorExamResultData[] }>);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>View your students' mock exam performance and rankings</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {Object.entries(resultsByStudent).map(([studentId, { studentName, exams }]) => (
                  ...
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        data-testid={`btn-tutor-view-results-${result.exam.id}`}
                      >
      ...
      <Dialog open={!!selectedResult} onOpenChange={(open) => !open && setSelectedResult(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{selectedResult?.exam.title} - {selectedResult?.studentName}</DialogTitle>
            <DialogDescription>
              {selectedResult && format(new Date(selectedResult.exam.examDate), "dd MMM yyyy")}
            </DialogDescription>
          </DialogHeader>

          {selectedResult?.statistics.map((stat) => {
            const paper = selectedResult.papers.find(p => p.id === stat.paperId);
              ...
              <div key={stat.paperId} className="space-y-4 border-b pb-6 last:border-b-0">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-primary/10 rounded-lg p-3 text-center">
                    ...
                    <p className="font-bold text-xl text-primary">
                      {stat.studentScore !== null ? stat.studentScore : "-"}
                    </p>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={[
                        { name: "Score", score: stat.studentScore || 0, fill: "hsl(var(--primary))" },
